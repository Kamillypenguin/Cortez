...
import { Icon, Sparkle, Logo, Avatar, Badge, StatusDot } from '../../components/ui'
...
const metrics = [
  ...
  { label: 'Prazo', value: 'Em risco', icon: 'alert-triangle', status: 'warn' as const },
  { label: 'Riscos abertos', value: '4', delta: '2 críticos', good: false, icon: 'flame', status: 'bad' as const },
]

const modules = [
  { name: 'Discovery & Research', pct: 100, status: 'ok' as const },
  { name: 'Design System', pct: 92, status: 'ok' as const },
  { name: 'Frontend — Web app', pct: 68, status: 'warn' as const },
  { name: 'Backend — APIs', pct: 71, status: 'ok' as const },
  { name: 'Integração SSO', pct: 35, status: 'bad' as const },
  { name: 'QA & Homologação', pct: 18, status: 'paused' as const },
]

const risks = [
  { sev: 'Crítico', tone: 'urgent' as const, title: 'Dependência de SSO sem owner', impact: 'Bloqueia GA · 8 dias' },
  { sev: 'Crítico', tone: 'urgent' as const, title: 'API de pagamento sem ambiente staging', impact: 'Sem teste end-to-end' },
  { sev: 'Atenção', tone: 'attention' as const, title: 'Time de Design com 1 vaga aberta', impact: 'Velocity -20% em 2 sprints' },
  { sev: 'Atenção', tone: 'attention' as const, title: 'Cliente solicitou 2 novos requisitos', impact: 'Escopo +12%' },
]

const audienceTexts: Record<string, { label: string; title: string; body: string }> = {
  brm: {
    label: 'BRM / Diretor',
...
export function StatusReport() {
  ...
  return (
    <div className="cortex" style={{ width: '100%', height: '100%', background: 'var(--bg-base)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>

      {/* Topbar */}
      <header style={{ height: 60, borderBottom: '1px solid var(--bd-default)', display: 'flex', alignItems: 'center', padding: '0 28px', gap: 16, background: 'var(--bg-surface)', flexShrink: 0 }}>
        <button className="icon-btn" onClick={() => setPage('dashboard')}><Icon name="arrow-left" size={16} /></button>
      ...
      <div className="scroll" style={{ flex: 1, overflow: 'auto', padding: '24px 28px 24px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            {['#6366F1', '#10B981', '#F59E0B', '#EC4899', '#3B82F6'].map((c, i) => (
              <div key={i} style={{ marginLeft: i > 0 ? -8 : 0 }}>
                <Avatar name={`P${i + 1}`} size={28} color={c} />
              </div>
            ))}
            ...
            return (
              <div key={i} className="card" style={{ padding: 16, position: 'relative', overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--tx-2)' }}>{m.label}</span>
                ...
                <div className="mono" style={{ fontSize: 28, fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 1, color: m.status ? statusColor : 'var(--tx-1)' }}>{m.value}</div>
                {m.delta && (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 22 }}>
          ...
          <div className="card" style={{ padding: 0 }}>
            <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--bd-default)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h3 style={{ margin: 0, fontSize: 14, fontWeight: 600 }}>Progresso por módulo</h3>
          ...
          <div className="card" style={{ padding: 0 }}>
            <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--bd-default)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h3 style={{ margin: 0, fontSize: 14, fontWeight: 600 }}>Riscos e impedimentos</h3>
        ...
        <div className="card" style={{ padding: 0, marginBottom: 24 }}>
          <div style={{ padding: '14px 18px 0', borderBottom: '1px solid var(--bd-default)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12 }}>
              <h3 style={{ margin: 0, fontSize: 14, fontWeight: 600 }}>Versões por audiência</h3>
          ...
          <div style={{ padding: 22, display: 'grid', gridTemplateColumns: '1fr 220px', gap: 24 }}>
            ...
            <div style={{ borderLeft: '1px solid var(--bd-default)', paddingLeft: 24, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--tx-3)', marginBottom: 6 }}>Tom</div>
                <div style={{ fontSize: 13, fontWeight: 500 }}>
                  {audience === 'brm' ? 'Executivo · direto' : audience === 'tech' ? 'Técnico · preciso' : 'Empático · transparente'}
                </div>
              </div>
              <div>
                <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--tx-3)', marginBottom: 6 }}>Tempo de leitura</div>
                <div style={{ fontSize: 13, fontWeight: 500 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 18px', background: 'var(--bg-card)', border: '1px solid var(--bd-default)', borderRadius: 12 }}>
          ...
          <button className="btn btn-primary" style={{ padding: '8px 20px' }}><Icon name="send" size={14} /> Confirmar envio</button>
        </div>
      </div>
    </div>
  )
