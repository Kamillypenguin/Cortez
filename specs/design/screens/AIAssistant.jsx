...
function ScreenAIAssistant() {
  ...
  return (
    <div className="cortex" style={{ width: 1440, height: 900, display: 'grid', gridTemplateColumns: '260px 1fr', background: 'var(--bg-base)', overflow: 'hidden' }}>

      {/* History sidebar */}
      <aside style={{ background: 'var(--bg-surface)', borderRight: '1px solid var(--bd-default)', padding: 14, display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 4px 16px' }}>
          <button className="icon-btn"><Icon name="arrow-left" size={16} /></button>
          <Logo size={22} withText={false} />
          <span style={{ fontWeight: 600, fontSize: 14 }}>Assistente</span>
        </div>
          ...
          {[
            ...
            },
            {
              ...
              items: [
                ...
                { id: 'c7', t: 'Revisar resumo do CV', sub: '2 mensagens' },
              ],
            },
          ].map(g => (
            <div key={g.date}>
              ...
              {g.items.map(it => (
                <button key={it.id}
                  ...
                  onMouseOver={e => { if (activeChat !== it.id) e.currentTarget.style.background = 'var(--bg-card)'; }}
                  onMouseOut={e => { if (activeChat !== it.id) e.currentTarget.style.background = 'transparent'; }}
                >
                  {it.sparkle ? <Sparkle size={14} style={{ marginTop: 2 }} /> : <Icon name="message" size={14} style={{ marginTop: 2, color: 'var(--tx-3)', flexShrink: 0 }} />}
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--tx-1)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{it.t}</div>
                    <div style={{ fontSize: 11, color: 'var(--tx-3)', marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{it.sub}</div>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div style={{ padding: '14px 6px 4px', borderTop: '1px solid var(--bd-default)', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 12, fontWeight: 500 }}>Mariana</div>
      ...
      <main style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {/* Header */}
        <header style={{ height: 56, borderBottom: '1px solid var(--bd-default)', display: 'flex', alignItems: 'center', padding: '0 28px', gap: 14 }}>
          <div style={{ flex: 1 }}>
                          ...
                          {m.card && (
                            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--c-primary-border)', borderRadius: 12, overflow: 'hidden', boxShadow: 'var(--sh-glow)' }}>
                              ...
                              <div style={{ padding: '10px 14px', background: 'rgba(0,0,0,0.2)', borderTop: '1px solid var(--bd-default)', display: 'flex', gap: 6 }}>
                                <button className="btn btn-sm btn-secondary"><Icon name="file-plus" size={12} /> Salvar como arquivo</button>
        ...
        {/* Input */}
        <div style={{ padding: '12px 28px 24px', background: 'var(--bg-base)' }}>
          <div style={{ maxWidth: 720, margin: '0 auto' }}>
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', padding: '4px 8px 8px', gap: 4 }}>
                ...
                <span style={{ flex: 1 }} />
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--tx-3)', marginRight: 6 }}>
                  <Icon name="cpu" size={12} />Cortex 4 Pro
                </div>
                <button onClick={send} className="btn btn-ai btn-sm" style={{ height: 32, padding: '0 14px' }}>
                  <Icon name="arrow-up" size={14} /> Enviar
                </button>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
              {[
                ...
                <button key={a.l} style={{
                  ...
                  borderRadius: 99, border: '1px solid var(--bd-default)',
                  background: 'var(--bg-surface)', color: 'var(--tx-2)', cursor: 'pointer',
                  transition: 'all 120ms',
                }}
                  onMouseOver={e => { e.currentTarget.style.borderColor = 'var(--c-primary)'; e.currentTarget.style.color = 'var(--tx-1)'; }}
                  onMouseOut={e => { e.currentTarget.style.borderColor = 'var(--bd-default)'; e.currentTarget.style.color = 'var(--tx-2)'; }}>
...
window.ScreenAIAssistant = ScreenAIAssistant;
